...
export default class TextSelect {
  ...
  handleTouchStart = (e) => {
    ...
    // 是否正在触摸左右cursor
    var touchStartCursor = e.target.classList.contains("mobile-text-selection-tools-cursor-start");
    var touchEndCursor = e.target.classList.contains("mobile-text-selection-tools-cursor-end");
    if (touchMenu) {
  ...
  handleTouchMove = (e) => {
    ...
    if (this.selectStatus === "selecting") {
      ...
      if (this.movingCursor) {
        ...
        if (target) {
          this.magnifier.show();
          console.log(target, mouseX, mouseY);
      ...
      if (node.nodeName === "#text") {
        ...
        for (let j = 0; j < content.length; j++) {
          ...
          if (
            ...
            const isLineEnd =
              !nextRect || (nextRect && isEndRect);
            console.log(nextRect, 'nextRect', nextRect.right, 'right', rect.left, 'left', rect, 'rect', x, 'x');
            if (isLineEnd) {
              return {
                x: rect.right,
                y: rect.top,
